...
fn sha256(bytes: &[u8]) -> [u8; 32] {
    ...
    for chunk in padded.chunks_exact(64) {
        ...
        for (word, input) in words.iter_mut().take(16).zip(chunk.chunks_exact(4)) {
            *word = u32::from_be_bytes([input[0], input[1], input[2], input[3]]);
        }
        for index in 16..64 {
        ...
        for (&constant, &word) in ROUND_CONSTANTS.iter().zip(words.iter()) {
            ...
            let temp2 = second.wrapping_add(majority);
            h = g;
            g = f;
            f = e;
    ...
    for (word, output) in state.iter().zip(result.chunks_exact_mut(4)) {
