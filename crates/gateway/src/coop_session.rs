...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct PeerBinding { role: CoopPeerRole, generation: u64, status: PeerStatus }
...
impl CoopSynchronizationSnapshot {
    #[must_use] pub const fn instance_id(&self) -> InstanceId { self.instance_id }
    #[must_use] pub const fn lease_epoch(&self) -> LeaseEpoch { self.lease_epoch }
    #[must_use] pub const fn generation(&self) -> u64 { self.generation }
    #[must_use] pub const fn peer_count(&self) -> usize { self.peer_count }
    #[must_use] pub fn missing_peers(&self) -> &[CallerId] { &self.missing_peers }
    #[must_use] pub const fn disagreement(&self) -> bool { self.disagreement }
    #[must_use] pub const fn mutation_allowed(&self) -> bool {
...
impl CoopSession {
    pub const fn new(instance_id: InstanceId, lease_epoch: LeaseEpoch, generation: u64) -> Self {
        Self { instance_id, lease_epoch, generation, peers: BTreeMap::new() }
    }
    ...
    pub fn register_peer(
        ...
        if role == CoopPeerRole::Local
            && self.peers.values().any(|peer| peer.role == CoopPeerRole::Local)
        {
            return Err(CoopSessionError::DuplicateLocalPeer);
        }
        self.peers.insert(peer_id, PeerBinding { role, generation, status: PeerStatus::Connected });
        Ok(())
    }

    pub fn update_generation(&mut self, peer_id: CallerId, generation: u64) -> Result<(), CoopSessionError> {
        if generation > MAX_GENERATION {
            return Err(CoopSessionError::GenerationOutOfBounds);
        }
        let peer = self.peers.get_mut(&peer_id).ok_or(CoopSessionError::UnknownPeer)?;
    ...
    pub fn reconnect(&mut self, peer_id: CallerId, generation: u64) -> Result<(), CoopSessionError> {
        if generation > MAX_GENERATION {
            return Err(CoopSessionError::GenerationOutOfBounds);
        }
        let peer = self.peers.get_mut(&peer_id).ok_or(CoopSessionError::UnknownPeer)?;
    ...
    pub fn authorize_mutation(&self, generation: u64) -> Result<(), CoopSessionError> {
        ...
        if generation != snapshot.generation { return Err(CoopSessionError::GenerationDisagreement); }
        if !self.peers.values().any(|peer| peer.role == CoopPeerRole::Local) {
            return Err(CoopSessionError::MissingLocalPeer);
        }
        if !snapshot.mutation_allowed() { return Err(CoopSessionError::MutationSuspended); }
        Ok(())
    }
    ...
    pub fn snapshot(&self) -> CoopSynchronizationSnapshot {
        ...
        for (peer_id, peer) in &self.peers {
            if peer.status == PeerStatus::Disconnected { missing_peers.push(*peer_id); }
            if peer.generation != self.generation { disagreement = true; }
        }
