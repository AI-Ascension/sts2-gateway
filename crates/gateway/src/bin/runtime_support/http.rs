...
pub(crate) fn read_response_with_limit(
    ...
    response::read_response_with_limit(stream, expires, max_response_bytes)
}

pub(crate) fn read_response_with_limit_cancelable<F>(
    stream: &mut TcpStream,
    expires: Instant,
    max_response_bytes: usize,
    should_cancel: F,
...
where
    F: Fn() -> bool,
{
    cancelable::read_response_with_limit(stream, expires, max_response_bytes, should_cancel)
}
...
pub(crate) fn write_request_cancelable<F>(
    stream: &mut TcpStream,
    method: &str,
    path: &str,
    headers: &BTreeMap<String, String>,
...
where
    F: Fn() -> bool,
{
    cancelable::write_request(stream, method, path, headers, body, expires, should_cancel)
}
